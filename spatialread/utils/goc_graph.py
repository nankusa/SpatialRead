...
def _generate_graph(
    data: BaseData, *, cutoff: float, max_neighbors: int, pbc: bool, VNODE_Z: int
):
    if pbc:
        # large cutoff and max_neighbors to ensure all atoms have neighbors

        edge_index, cell_offsets, neighbors = radius_graph_pbc(
            data,
            radius=cutoff,
            max_num_neighbors_threshold=max_neighbors + 30,
            # pbc=[True, True, True],
            # rep=[2, 2, 2]
        )
        ...
        # print("DEBUG EDGE DIST", edge_dist.min(), edge_dist.max(), edge_dist.mean(), edge_dist.std())

        edge_index, edge_dist, distance_vec, [cell_offsets, cell_offset_distances] = (
            filter_edge(
                edge_index,
                edge_dist,
                distance_vec,
                "p2pv",
                data.atomic_numbers == VNODE_Z,
                cutoff,
                max_neighbors,
                other_content=[cell_offsets, cell_offset_distances],
            )
        )
        # print("DEBUG", edge_index.shape, edge_dist.shape, distance_vec.shape, cell_offsets.shape, cell_offset_distances.shape)
    else:
...
def generate_graph(
    ...
    *,
    ...
    (
        ...
        data, cutoff=cutoff, max_neighbors=max_neighbors, pbc=pbc, VNODE_Z=120
    )
...
def _subselect_edges(
    ...
    if max_neighbors is not None:
        ...
        edge_mask, subgraph["num_neighbors"] = get_max_neighbors_mask(
            natoms=(
                torch.tensor([data.natoms], dtype=torch.long, device=data.pos.device)
